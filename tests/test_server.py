...
def test_main_run_ignores_oauth_env_for_public_finals_server(monkeypatch, tmp_path):
    ...
    assert not any(item.cls is main.MCPSelectiveAuthMiddleware for item in middleware)


def test_public_mcp_tools_and_calls_do_not_require_auth(cache, monkeypatch, tmp_path):
    """본선용 비인증 MCP는 tools/list와 tools/call 모두 인증 없이 동작한다."""
